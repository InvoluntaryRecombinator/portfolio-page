      ...
      execution: [
        {
          ...
          description:
            "The packet is filled and assembled in-browser through a JavaScript PDF library (pdf-lib); documents are built in memory and never sent to a server. Working state is held in local storage rather than a backend, so a session persists across reloads with no account or login. A single click wipes local state for users on borrowed or public devices, and progress can be carried between sessions by exporting a JSON file that stores the user's data and re-uploading it later to resume. Handling sensitive criminal histories and PII with no server-side storage — keeping the process revisitable and transferable without auth.",
        },
        {
          title: "Minimized Request Boundary",
          description:
            "The interview request is the only user data that leaves the device. It's assembled from a single incident rather than the applicant record, so the request object carries the offense, sentence, disposition, year, and state — and structurally cannot include the name, address, DOB, or exact dates. A server-side guard rejects identity-shaped fields before the request reaches the model, and a leak test serializes the outgoing object to assert the excluded fields are absent.",
        },
        {
          title: "Structured-Output Interview",
          description:
            "An LLM conducts the interview, but the model can't be trusted to run the flow or stay truthful on its own — so it's built on the OpenAI API's structured-output mode, where every turn returns a typed JSON object instead of free text. The code reads that object to control the conversation: what to ask next, what's still thin, and whether enough has been gathered. The model only interprets; the application logic decides. When the state shows the account is complete, the flow switches to a dedicated draft_now step rather than letting the model decide it's done — and before any generated text is accepted, server-side guards reject invented facts, wrong conviction terminology, and anything the user didn't actually say.",
        },
        {
          title: "Verified Form Mapping",
          description:
            "The government forms use opaque, inconsistent AcroForm conventions — machine-generated field names, and checkbox values where the same code means \"yes\" on one field and \"no\" on another. The field map was derived by probing the real forms directly and validated against them so every value writes correctly, rather than trusting the field names to mean what they appear to. Data is then written into that verified map to populate each form.",
        },
      ],
    },
  },
